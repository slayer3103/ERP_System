...
const handleCopyBilling = (checked) => {
  ...
  const handleSave = async () => {
    // ── Frontend business validations ─────────────────────────────
    if (!vendorName.trim())        { alert('Vendor name is required'); return; }
    if (!companyPhone.trim())      { alert('Phone number is required'); return; }
    if (!billingAddress1.trim())   { alert('Billing address is required'); return; }
    if (!billingCity.trim())       { alert('Billing city is required'); return; }
    if (!billingState.trim())      { alert('Billing state is required'); return; }
    if (!billingPinCode.trim())    { alert('Billing pincode is required'); return; }
    if (!gstNumber.trim())         { alert('GST number is required for vendors'); return; }
    ...
    if (!/^\d{6}$/.test(billingPinCode)) {
      alert('Billing pincode must be exactly 6 digits'); return;
    }
    if (panNumber && !/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(panNumber.toUpperCase())) {
      alert('Invalid PAN format (e.g. ABCDE1234F)'); return;
    }
    if (!/^\d{2}[A-Z]{5}\d{4}[A-Z]{1}[A-Z\d]{1}[Z]{1}[A-Z\d]{1}$/.test(gstNumber.toUpperCase())) {
      alert('Invalid GST format (e.g. 27ABCDE1234F1Z5)'); return;
    }
    // GST PAN cross-check
    if (gstNumber && panNumber) {
      const panInGst = gstNumber.substring(2, 12).toUpperCase();
      if (panInGst !== panNumber.toUpperCase()) {
        alert('PAN embedded in GST does not match the PAN field'); return;
      }
    }
    // Bank details: all or none
    const hasBankField = accountNumber || ifscCode || bankName || accountHolderName;
    if (hasBankField) {
      if (!accountHolderName.trim()) { alert('Account holder name is required'); return; }
      if (!bankName.trim())          { alert('Bank name is required'); return; }
      if (!accountNumber.trim())     { alert('Account number is required'); return; }
      if (!ifscCode.trim())          { alert('IFSC code is required'); return; }
      if (!/^\d{9,18}$/.test(accountNumber)) { alert('Account number must be 9-18 digits'); return; }
      if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifscCode.toUpperCase())) { alert('Invalid IFSC code (e.g. SBIN0001234)'); return; }
    }
