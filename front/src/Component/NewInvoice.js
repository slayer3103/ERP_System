...
const NewInvoicePage = () => {
  ...
  const handleSubmit = async (saveAsDraft = false) => {
    ...
    if (!customerObj) { alert('Please select a customer'); return; }
    if (!invoiceDate)  { alert('Invoice date is required'); return; }
    if (!expiryDate)   { alert('Due date is required'); return; }
    if (new Date(expiryDate) < new Date(invoiceDate)) {
      alert('Due date must be on or after invoice date'); return;
    }
    if (rows.length === 0) { alert('Please add at least one item'); return; }

    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      if (!row.item)              { alert(`Select an item for row ${i + 1}`); return; }
      if (!row.qty || row.qty <= 0) { alert(`Quantity must be greater than 0 for row ${i + 1}`); return; }
      if (!row.rate || row.rate <= 0) { alert(`Rate must be greater than 0 for row ${i + 1}`); return; }
      if (row.discount < 0)       { alert(`Discount cannot be negative for row ${i + 1}`); return; }
    }
    if (parseFloat(freight) < 0) { alert('Freight cannot be negative'); return; }
